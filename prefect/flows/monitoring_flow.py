...
def system_health_check_flow() -> dict:
    ...
    if report['overall_status'] != 'healthy':
        print("\nSystem health issues detected - sending alert...")
...
def data_freshness_monitor_flow(alert_threshold_hours: int = 48) -> dict:
    print(f"\nData Freshness Monitor (threshold: {alert_threshold_hours}h)\n")
    
    freshness = check_data_freshness_task()
    
    if freshness['stale_channels'] > 0:
        print(f"\nFound {freshness['stale_channels']} stale channels")
...
Please review and trigger manual crawl if needed.
        """
...
def api_quota_monitor_flow(warning_threshold: float = 80.0) -> dict:
    print(f"\nAPI Quota Monitor (warning at {warning_threshold}%)\n")
    
    quota = check_api_quota_task()
    
    if quota['percentage_used'] >= warning_threshold:
        print(f"\nAPI quota at {quota['percentage_used']:.1f}%")
...
- Limiting channels to crawl
- Waiting for quota reset (00:00 UTC)
        """
        
        send_email_notification_task(subject, body)
    else:
        print(f"Quota usage: {quota['percentage_used']:.1f}%")
